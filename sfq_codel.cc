...
/* TODO: Argument is in fact string */
void SfqCoDel::enqueue(const string & contents) {
  /* Implements pure virtual function Queue::enque() */
  uint64_t bin_id = hash( QueuedPacket( contents ) );
...
uint64_t SfqCoDel::hash( const string & contents ) const {
...
void SfqCoDel::enque_packet(QueuedPacket p, uint64_t bin_id) {
...
QueuedPacket SfqCoDel::codel_deq_helper(uint64_t bin_id) {
  if (!pkt_queues_.at(bin_id).empty()) {
    ...
    return QueuedPacket::nullpkt(); /* TODO: Ensure this is correct */
  }
}
...
QueuedPacket SfqCoDel::codel_deque(uint64_t bin_id) {
...
void SfqCoDel::drop(QueuedPacket p, uint64_t bin_id) {
